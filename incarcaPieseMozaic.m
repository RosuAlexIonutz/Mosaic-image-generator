...
numeDirector = 'Airplane'
fisiere = dir([params.numeDirector '/*.png'])
fprintf('dimensiune fisiere %d \n', length(fisiere));
for idx = 1 : length(fisiere)
    if fisiere(idx).isdir == 0
        pieseMozaic(:,:,:,idx) = imread([ params.numeDirector '/' fisiere(idx).name ]);
    end
end
